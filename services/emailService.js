  ...
  switch (status) {
    ...
    case 'released':
      return releaseMethod === 'online'
        ...
        : 'Your document has been released. Please visit the CEAT OCS office to pick it up.';
    case 'rejected':
...
const sendDocumentUpdateEmail = async ({ studentName, studentEmail, trackingCode, status, releaseMethod, remark }) => {
  ...
  const html = `
    <div style="font-family: 'Helvetica Neue', Arial, sans-serif; max-width: 560px; margin: 0 auto; background: #f8fafc; padding: 24px;">
      <div style="background: #7b1113; border-radius: 10px 10px 0 0; padding: 24px 28px; text-align: center;">
        <h2 style="color: #fff; margin: 0; font-size: 18px; font-weight: 700;">CEAT Office of the College Secretary</h2>
        <p style="color: rgba(255,255,255,0.8); margin: 4px 0 0; font-size: 13px;">Document Tracking System</p>
      </div>
      <div style="background: #fff; padding: 28px; border: 1px solid #e5e7eb; border-top: none; border-radius: 0 0 10px 10px;">
        <p style="color: #374151; font-size: 15px; margin: 0 0 16px;">Dear <strong>${studentName}</strong>,</p>
        <p style="color: #374151; font-size: 14px; margin: 0 0 20px; line-height: 1.6;">This is an update regarding your document request.</p>
        <div style="background: #f8fafc; border: 1px solid #e5e7eb; border-radius: 8px; padding: 16px; margin-bottom: 20px;">
          ...
          <p style="margin: 0; font-size: 18px; font-weight: 700; color: #111827;">${trackingCode}</p>
        </div>
        <div style="background: ${color.bg}; border: 1px solid ${color.border}; border-left: 4px solid ${color.border}; border-radius: 8px; padding: 14px 16px; margin-bottom: 20px;">
          <p style="margin: 0 0 4px; font-size: 11px; font-weight: 700; color: ${color.text}; text-transform: uppercase;">Current Status</p>
        ...
        <div style="background: #fef6e0; border: 1px solid #f5a800; border-left: 4px solid #f5a800; border-radius: 8px; padding: 14px 16px; margin-bottom: 20px;">
          <p style="margin: 0 0 6px; font-size: 11px; font-weight: 700; color: #7a4f00; text-transform: uppercase;">Remarks from Staff</p>
          <p style="margin: 0; font-size: 14px; color: #374151; line-height: 1.6;">${remark}</p>
        </div>
        ` : ''}
        <div style="text-align: center; margin: 24px 0;">
          <a href="${process.env.CLIENT_URL}" style="display: inline-block; background: #7b1113; color: #fff; text-decoration: none; padding: 12px 28px; border-radius: 8px; font-size: 14px; font-weight: 600;">
            View Document Status
          </a>
        </div>
        <p style="color: #374151; font-size: 14px; margin: 0 0 4px;">If you have any questions, please contact the CEAT OCS office.</p>
  ...
  await resend.emails.send({
    from: 'CEAT OCS DTS <onboarding@resend.dev>',
    to: studentEmail,
    subject: `[${trackingCode}] Document Status Update — ${statusLabel}`,
    html,
  });
};
